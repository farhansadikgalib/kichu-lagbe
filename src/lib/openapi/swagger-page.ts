...
/**
 * Interactive API reference (Swagger UI) for a given OpenAPI document. Plain
 * HTML from a route handler — no React or bundle cost; the UI loads from a
 * CDN. "Try it out" sends the browser's own session cookie, so sign in
 * through the login operation first and every protected call works from here.
 */
export function renderSwaggerPage({ pageTitle, specUrl }: { pageTitle: string; specUrl: string }) {
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
...
export function swaggerResponse(html: string) {
